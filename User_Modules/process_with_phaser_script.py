...
# Basically, this script works on 2D/3D data, but 1D can be viewed from phaser. 
...
for dim in range(ndim):
  ...
  if 'auto' in p_list[dim]:
    ...
    xdata = uc.ppm_scale()
    d = pokyphaser.show_phaser(s)
    # d.set_data(data, xdata, index=0) # when using only first slice
    d.set_data(data, xdata, index='sum')
