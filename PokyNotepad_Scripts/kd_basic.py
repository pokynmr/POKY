...
print('\n\n\n------------------------------------------------------')
print('POKY Kd prediction by CSP')
...
# If you are running many peaks, this can be risky
plot = True
...
if len(Kd_list) != 0:
  print('-------------------------')
  print('Max: %.4f' % (max(Kd_list)) + unit)
  print('Min: %.4f' % (min(Kd_list)) + unit)
  print('Avg: %.4f %s (+/- %.4f)' %
        (sum(Kd_list) / len(Kd_list), unit, np.std(Kd_list)))
  print('-------------------------')
